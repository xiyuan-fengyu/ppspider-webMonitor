...
class DevTools {
  ...
  static rebind(clazz, funName, newFun) {
    const oldFun = clazz.prototype[funName];
    clazz.prototype[funName] = function (...args) {
        newFun(this, oldFun, ...args);
    };
  }
  ...
  monkeypatchTimelineFeatures() {
    ...
    UI.inspectorView.showPanel('timeline').then(_ => {
        // 解决滚动崩溃的问题
        let lastCallTime = 0;
        DevTools.rebind(Timeline.PerformanceModel, "dispatchEventToListeners", (target, oldFun, ...args) => {
          if (args[1].window.left != null) {
            const now = new Date().getTime();
            if (now - lastCallTime >= 5) {
              lastCallTime = now;
              oldFun.call(target, ...args);
            }
          }
        });

        const networkFlameChart = Timeline.TimelinePanel.instance()._flameChart._networkFlameChart;
        const old = networkFlameChart.setWindowTimes;
        networkFlameChart.setWindowTimes = (...args) => {
            if (!isNaN(args[0])) {
                old.call(networkFlameChart, ...args);
            }
        };

      // 将network面板最大化
      const networkSplitWidget = Timeline.TimelinePanel.instance()._flameChart._networkSplitWidget;
      const oldOnLayout = networkSplitWidget.onLayout;
      networkSplitWidget.onLayout = () => {
        oldOnLayout.call(networkSplitWidget);
        setTimeout(() => {
          networkSplitWidget._onResizeUpdate({data: {startPosition: 0, currentPosition: 10000}});
        }, 30);
      };
